<template>
  <div>
    ...
    <div class="row hidden-xs">
      ...
      <!-- update 2023.12.31 添加上次浏览记录窗口 -->
      <div
        class="screen-box-card"
        v-if="!loopPlaying && hasLocalData() && showlocalDataCard"
      >
        <div class="title">是否打开上次浏览的视频</div>
        <div class="footer">
          <el-button size="mini" @click="hideLocalDataCard">取消</el-button>
          <el-button
            size="mini"
            type="primary"
            @click.prevent="loadLocalData(true)"
          >
            确认
          </el-button>
        </div>
      </div>
      <!-- update 2023.12.31 添加收藏夹窗口 -->
      <Favorites
        :showFavoritesCard="showFavoritesCard"
        @handleShowFavoritesCard="handleShowFavoritesCard"
      />
      <div class="text-center">
        <el-button-group class="player-btn-group">
          ...
          <el-button
            ...
            v-if="!isIE()"
            @click.prevent="
              loopPlaying ? stopLoopPlay() : showLoopPlayListDlg()
            "
            :title="loopPlaying ? '停止轮巡播放' : '选择轮巡播放通道'"
          >
    ...
    <div class="view-list row">
      <div class="col-md-3" id="dev-tree-left">
        ...
        <ul class="nav nav-tabs">
          <li class="active" @click="switchTab('dev')">
            <a href="#dev-tree-wrapper" data-toggle="tab">设备树</a>
          </li>
          <li @click="switchTab('group')">
            <a href="#group-tree-wrapper" data-toggle="tab">分 组</a>
          </li>
          <li @click="switchTab('map')">
            <a href="#group-tree-wrapper" data-toggle="tab">地图选择</a>
          </li>
        </ul>
        <div class="tab-content" style="margin: 10px 0" id="tab-tree-wrapper">
          ...
          >
            <el-tree
              ...
              id="query-dev-tree"
              node-key="key"
              v-if="showQueryDevTree"
              :data="queryDevTreeDataList"
              :props="treeProps"
              :load="queryDevTreeLoad"
              lazy
              :empty-text="queryDevTreeEmptyText"
            ...
            >
              <span class="custom-tree-node" slot-scope="{ node, data }">
                <i
                  ...
                  :style="
                    treeLeaf(data)
                      ? 'color:#c0c4cc;font-size:12px;margin-left:-16px;'
                      : 'color:#c0c4cc;font-size:12px;'
                  "
                  title="点击查询上一层"
                  @click.stop="q = 'code:' + data.parentID"
            ...
            <el-tree
              ...
              :props="treeProps"
              :load="treeLoad"
              :filter-node-method="devTreeFilter"
              lazy
          ...
          <div class="tab-pane" ref="groupTreeWrapper" id="group-tree-wrapper">
            <el-tree
              ...
              id="query-group-tree"
              node-key="key"
              v-if="showQueryGroupTree"
              :data="queryGroupTreeDataList"
              :props="treeProps"
              :load="queryGroupTreeLoad"
              lazy
              :empty-text="queryGroupTreeEmptyText"
              :draggable="
                hasAnyRole(serverInfo, userInfo, '管理员', '超级管理员') &&
                  !isMobile()
              "
                ...
                >
                  ...
                  <span
                    :class="[
                      'ellipsis',
                      {
                        'text-bold': !!data.custom,
                        'text-under-dashed':
                          node.parent &&
                          node.parent.data &&
                          node.parent.data.custom === false,
                      },
                    ]"
                    style="user-select: none"
                    :title="node.label"
            ...
            <el-tree
              ...
              node-key="key"
              v-if="showGroupTree"
              v-show="!showQueryGroupTree"
              :props="treeProps"
              :load="groupTreeLoad"
              :filter-node-method="groupTreeFilter"
              lazy
              :empty-text="groupTreeEmptyText"
              :default-expanded-keys="defExpandGroups"
              :draggable="
                hasAnyRole(serverInfo, userInfo, '管理员', '超级管理员') &&
                  !isMobile()
              "
      ...
      >
        <a
          role="button"
          @click="playStop(contextMenuNode)"
          v-show="
            contextMenuNodeData &&
              !contextMenuNodeData.custom &&
              contextMenuNodeData.code &&
              contextMenuNode &&
              contextMenuNode.play &&
              !contextMenuNode.playing
          "
        >
        ...
        <a
          role="button"
          @click="treeNodeRefresh(pnode)"
          v-show="contextMenuNodeData"
        >
          <i class="fa fa-refresh"></i> 刷新节点
        </a>
        <a
          role="button"
          @click="showNodeAddDlg"
          v-show="
            contextMenuNodeData &&
              contextMenuNodeData.custom &&
              (contextMenuNodeData.code ||
                hasAllChannel(serverInfo, userInfo)) &&
              userInfo &&
              hasAnyRole(serverInfo, userInfo, '管理员', '超级管理员')
          "
        >
        ...
        <a
          ...
          v-show="
            contextMenuNodeData &&
              !contextMenuNodeData.custom &&
              contextMenuNodeData.manufacturer === 'virtual' &&
              contextMenuNodeData.code &&
              userInfo &&
              hasAnyRole(serverInfo, userInfo, '超级管理员')
          "
        >
        ...
        <a
          role="button"
          @click="
            showScreenChannelListDlg(contextMenuNodeData, contextMenuNode, c)
          "
          v-show="contextMenuNodeData && contextMenuNodeData.parental"
        >
          <i class="fa fa-edit"></i> 一键预览
        </a>
        <a
          role="button"
          @click="showPositionEditDlg"
          v-show="
            contextMenuNodeData &&
              !contextMenuNodeData.custom &&
              contextMenuNodeData.code &&
          ...
          <div class="video-show col-md-9 col-lg-10">
            <div>
              ...
              >
                <LivePlayer
                  :videoUrl="player.url"
                  :waterMark="player.osd"
                  :hideBigPlayButton="
                    !!serverInfo.HideBigPlayButton || loopPlaying
                  "
                  :smart="player.bSmart"
                  :poster="player.poster"
                  :controls="player.bControls && !loopPlaying"
                  custom-buttons="历史,配置:vjs-icon-cog"
                  @customButtons="customButtons($event, player, index)"
                  live
                  muted
                  stretch
                  @fullscreen="onFullscreenChange(player, index, $event)"
                  @media_info="onMediaInfo(player, index, $event)"
                ...
                >
                  <div
                    class="ptz-block ptz-block-fs"
                    v-show="
                      !isMobile() &&
                        hasAnyRole(serverInfo, userInfo, '管理员', '操作员') &&
                        player.bFullscreen &&
                        player.bControls
                    "
                  >
                    <div
                      class="ptz-cell ptz-up"
                    ...
                    <div
                      class="ptz-cell ptz-talk"
                      v-if="canTalk() && serverInfo.VersionType != '标准版'"
                    >
                      <i
                        class="fa fa-microphone"
                        @mousedown.prevent="talkStart"
                      ></i>
                    </div>
                    <div
                      class="ptz-cell ptz-zoomout"
                      @mousedown.prevent="ptzControl"
                      command="zoomout"
                      title="缩小"
                    >
                ...
                <div
                  ...
                  v-on:click="closeVideo(index, true)"
                >
                  关闭
                </div>
                <div
                  class="video-close channel-selector"
                  v-show="!player.url && player.bCloseShow && !loopPlaying"
                  v-on:click="selectChannel(index, player)"
                >
                  选择通道
                </div>
              </div>
              <ScreenChannelListDlg
                ref="channelListFullscreenDlg"
                @selected="play"
            ...
            <div
              class="text-center"
              v-if="isDemoUser(serverInfo, userInfo) && outHevcTipIdx < 0"
            >
              <br />
            ...
            <div class="text-center text-red" v-if="outHevcTipIdx >= 0">
              <br />
              提示: 正在播放 H265 FLV 直出流, 确保浏览器版本较新,
              并且开启硬件加速
            </div>
          </div>
          <div
            ...
            id="dev-tree-ptz"
            v-if="hasAnyRole(serverInfo, userInfo, '管理员', '操作员')"
          >
            <div class="ptz-block">
              <el-progress
                class="mic-level-bar"
              ...
              <div
                class="ptz-cell ptz-up"
                :class="{ readonly: !playing || loopPlaying }"
                command="up"
                title="上"
                @mousedown.prevent="ptzControl"
                @touchstart.prevent="ptzControl"
              >
                <i class="fa fa-chevron-up"></i>
              </div>
              <div
              ...
              <div class="ptz-cell ptz-center" title="云台控制">
                <i
                  ...
                  title="按住喊话"
                  @mousedown.prevent="talkStart"
                  @touchstart.prevent="talkStart"
                  v-if="canTalk() && serverInfo.VersionType != '标准版'"
                ></i>
                <i
                  class="fa fa-microphone-slash"
                  :class="{ readonly: !playing || loopPlaying }"
                  title="由于浏览器安全策略, 非 HTTPS 或 localhost 访问, 对讲不可用"
                  v-if="!canTalk() && serverInfo.VersionType != '标准版'"
                ></i>
              </div>
              <div
    ...
    <UploadDlg
      ref="uploadDlg"
      title="上传分组列表"
      url="/api/v1/channel/group/import"
      @uploaded="uploaded"
    ></UploadDlg>
    <ScreenChannelListDlg
      ref="channelListDlg"
    ...
    <ScreenLoopPlayListDlg
      ref="loopPlayListDlg"
      @loopPlay="startLoopPlay"
    ></ScreenLoopPlayListDlg>
    <DeviceTreeNodeEditDlg
      ref="nodeEditDlg"
      @submit="treeNodeRefresh(pnode)"
    ></DeviceTreeNodeEditDlg>
    <ChannelPositionEditDlg
      ref="positionEditDlg"
      @submit="treeNodeRefresh(pnode)"
    ></ChannelPositionEditDlg>
    <DeviceTreeNodeShareDlg
      ref="nodeShareDlg"
      :vtype="serverInfo.VersionType"
    ></DeviceTreeNodeShareDlg>
    <ChannelCustomListDlg
      ref="customListDlg"
      @hide="treeNodeRefresh(pnode)"
      :title="customListDlgTitle"
    ></ChannelCustomListDlg>
    <ScreenHistoryDlg
      :visible="showHistoryCard"
      :activeHistoryCardData="activeHistoryCardData"
      @handleShowHistoryCard="handleShowHistoryCard"
    />
  </div>
</template>
...
import ScreenHistoryDlg from "./ScreenHistoryDlg.vue";
...
export default {
  ...
  data() {
    return {
      ...
      playerLength: 4,
      loadedData: false,
      showlocalDataCard: true, // update 2023.12.31 展示上次浏览数据卡片
      showFavoritesCard: true, // update 2023.12.31 展示收藏夹卡片
      showHistoryCard: false, // update 2023.12.31 展示历史卡片
      activeHistoryCardData: {}, // update 2023.12.31 当前选中的历史卡片数据
      favoritesTreeData: [], // update 2023.12.31 收藏夹数据
      localData: {
        num1: {},
        num4: {},
        num9: {},
  ...
  mounted() {
    ...
    this.protocol = this.getQueryString(
      "protocol",
      this.serverInfo.PreferStreamFmt || ""
    );
  ...
  methods: {
    ...
    loadLocalData(update) {
      ...
      if (update) {
        if (
          this.isDemoUser(this.serverInfo, this.userInfo) &&
          this.playerLength > 4
        ) {
          this.$message({
            type: "error",
        ...
        this.loadedData = true;
        this.showlocalDataCard = false;
        for (let index = 0; index < this.playerLength; index++) {
          if (this.localData["num" + this.playerLength]["c" + index]) {
            var c = JSON.parse(
              this.localData["num" + this.playerLength]["c" + index]
            );
    ...
    onError(player, idx, e) {
      if (
        e == "MediaError" &&
        player.mediaInfo &&
        String(player.mediaInfo.videoCodec).startsWith("hvc")
      ) {
    ...
    onFullscreenChange(player, idx, bFullscreen) {
      if (player) {
        ...
        if (bFullscreen) {
          $(
            `#dev-tree-right .video-show .video:eq(${idx}) .ptz-block-fs`
          ).draggable({
            handle: ".ptz-center",
            cancel: ".ptz-cell",
    ...
    resetCloseTimer(player) {
      ...
      player.closeTimer = setTimeout(() => {
        ...
        if (
          $(this.$el).find(".ptz-cell.active").length == 0 &&
          $(this.$el).find(".fa-microphone.active").length == 0
        ) {
          player.bControls = false;
        }
      ...
      $.get("/api/v1/device/grouptree", {
          ...
          resolve(
            (ret || []).map((v, idx, arr) => {
              if (
                !node.level &&
                v.id &&
                this.defExpandGroups.indexOf(v.id) < 0
              ) {
        ...
        .always(() => {
          ...
          if (
            this.q &&
            !this.q.endsWith(" ") &&
            this.q.split(/\s+/).length === node.level - 1 &&
            this.groupLevelFilter
          ) {
            this.q += " ";
            this.$refs["q"].focus();
          }
    ...
    showNodeEditDlg() {
      ...
      var data = Object.assign({}, this.contextMenuNodeData, {
    ...
    // 一键预览
    showScreenChannelListDlg(data, node, c) {
      console.log("showScreenChannelListDlg", data, node, c);
      c.$refs.node.click();
      setTimeout(() => {
        node.childNodes.forEach((childNode) => {
          console.log(childNode);
          this.treeNodeClick(childNode.data, childNode);
        });
      }, 1000);
    },
    showPositionEditDlg() {
    ...
    showCustomListDlg() {
      ...
      this.customListDlgTitle = `选择通道(${this.contextMenuNodeData
        .customName ||
        this.contextMenuNodeData.name ||
        this.contextMenuNodeData.code})`;
    ...
    removeCustomNode() {
      ...
      if (!this.contextMenuNodeData) return;
      var name =
        this.contextMenuNodeData.customName ||
        this.contextMenuNodeData.name ||
        this.contextMenuNodeData.id;
    ...
    ptzControl(e) {
      ...
      $.ajax({
        ...
        if (xhr) {
          console.log(
            `ptz ${cmd} ajax error: ${xhr.status} ${xhr.responseText}`
          );
          if (xhr.status == 401) {
            location.href = `/login?r=${encodeURIComponent(
      ...
      if ($(this.$el).find(".ptz-cell.active").length > 0) {
        ...
        $.ajax({
          ...
          .fail((xhr) => {
            if (xhr) {
              console.log(
                `ptz stop ajax error: ${xhr.status} ${xhr.responseText}`
              );
              if (xhr.status == 401) {
                location.href = `/login?r=${encodeURIComponent(
                  window.location.href
    ...
    async loopPlay() {
      ...
      if (playerIdx == 0 && this.loopPlayerIndex > 0) {
        // 播放器复始
        this.loopPlayerIndex = 0;
        if (channelIdx > 0 || this.loopChannelIndex == 0) {
          // 点位没有复始
      ...
      return new Promise((resolve, reject) => {
        ...
        $.ajax({
          ...
          .fail((xhr) => {
            if (xhr) {
              console.log(
                `stream start in loop play error: ${xhr.status} ${xhr.responseText}`
              );
              if (xhr.status == 401) {
                this.$nextTick(() => {
                  location.href = `/login?r=${encodeURIComponent(
    ...
    switchTab(tab) {
      if (tab == "group" && !this.showGroupTree) {
        // lazy load group tree
        this.showGroupTree = true;
      }
    ...
    allowDrop(draggingNode, dropNode, type) {
      ...
      if (!draggingNode.parent || !draggingNode.data || !draggingNode.data.code)
        return false;
      if (!dropNode.data || !dropNode.data.custom) return false;
      if (!draggingNode.data.custom && !dropNode.data.code) return false;
      if (
        !dropNode.data.code &&
        !this.hasAnyRole(this.serverInfo, this.userInfo, "超级管理员")
      )
        return false;
      if (type == "inner") {
        return true;
    ...
    handleShowFavoritesCard() {
      this.showFavoritesCard = !this.showFavoritesCard;
    },
    customButtons(type, player, index) {
      console.log(type, player, index);
      if (type === "历史") {
        this.handleShowHistoryCard(player);
      }
    },
    handleShowHistoryCard(player) {
      this.activeHistoryCardData = player;
