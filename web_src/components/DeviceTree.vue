      ...
      <div class="box-body">
        ...
        <div class="content">
          ...
          <VueContextMenu class="right-menu" :target="contextMenuTarget" :show="contextMenuVisible" @update:show="(show) => contextMenuVisible = show">
            <a role="button" @click="treeRefresh" v-show="contextMenuNodeData">
              <i class="fa fa-refresh"></i> 刷新节点
            </a>
            <a role="button" @click="showNodeAddDlg" v-show="contextMenuNodeData && contextMenuNodeData.custom">
              <i class="fa fa-plus"></i> 新建节点
            </a>
            <a role="button" @click="showCustomListDlg" v-show="contextMenuNodeData && contextMenuNodeData.custom && contextMenuNodeData.code">
              <i class="fa fa-check"></i> 选择通道
            </a>
            <a role="button" @click="showNodeEditDlg" v-show="contextMenuNodeData && !(contextMenuNodeData.custom && !contextMenuNodeData.code)">
              <i class="fa fa-edit"></i> 编辑节点
            </a>
            <a role="button" @click="removeCustomNode" v-show="contextMenuNodeData && contextMenuNodeData.custom && contextMenuNodeData.code">
                ...
                <div class="video-show">
                  <div>
                    <div class="no-margin no-padding video col-sm-6" @click="clickPlayer(player, index, $event)" :class="{'active': index == playerIdx, 'col-sm-12': playersLength == 1,'col-sm-6': playersLength == 4,'col-sm-4': playersLength == 9,'col-sm-3': playersLength == 16}" v-for="(player,index) in players" :key="index" @mousemove="resetCloseTimer(player)" @touchstart="resetCloseTimer(player)">
                      <LivePlayer :videoUrl="player.url" :poster="player.poster" live muted stretch
            ...
            <div class="col-md-3 col-lg-2" id="dev-tree-ptz" v-if="hasAnyRole(serverInfo, userInfo, '管理员', '操作员')">
              <div class="ptz-block">
                  ...
                  <div class="ptz-cell ptz-center" title="云台控制">
                      <i class="fa fa-microphone" :class="{'readonly': !playing }" title="按住喊话" @mousedown.prevent="talkStart" v-if="canTalk() && serverInfo.VersionType != '标准版'"></i>
                      <i class="fa fa-microphone-slash" :class="{'readonly': !playing }" title="由于浏览器安全策略, 非 HTTPS 或 localhost 访问, 对讲不可用" v-if="!canTalk() && serverInfo.VersionType != '标准版'"></i>
                  </div>
...
export default {
    ...
    return {
      ...
      treeProps: {
        label: (data, node) => {
          ...
          if(!this.treeLeaf(data) && data.subCount > 0) {
  ...
  mounted() {
    ...
    $("#dev-tree-right").sticky({ topSpacing: 40, bottomSpacing: 80 })
  },
  beforeDestroy() {
    this.ctrlStop();
